...
const props = defineProps<{ prompt: string; busy: boolean; status: any; context: any }>();
...
<template>
<div class="image-prompt-backdrop" @click.self="emit('close')">
  <section class="image-prompt-modal">
    <header><div><p class="eyebrow">{{ tr('imagePromptEyebrow') }}</p><h2>{{ tr('checkImagePrompt') }}</h2></div><button type="button" :aria-label="tr('close')" @click="emit('close')">×</button></header>
