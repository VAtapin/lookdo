    ...
    <main class="register-main"><div class="register-card"><div class="mobile-step">{{ step }} / 4</div>
      <form v-if="step === 1" @submit.prevent="nextAccount">
        ...
        <div class="form-grid">
          ...
          <label>{{ tr('email') }}<input v-model="form.email" required type="email" autocomplete="email" @blur="checkAvailability(false)"><small v-if="form.email && availability.email" :class="availability.email.available ? 'field-ok' : 'field-error'">{{ availability.email.message }}</small></label>
