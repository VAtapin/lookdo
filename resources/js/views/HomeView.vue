...
<template>
  <div class="public-home">
      ...
      <div id="demo" class="public-phone-stage">
        ...
        <div class="app-phone request-phone">
          ...
          <div class="phone-screen customer-app-screen">
            <header class="phone-app-header"><span class="phone-mini-logo"><img :src="'/brand/lookdo-mark.png'" alt="" aria-hidden="true"></span><b>LOOK.<em>DO.</em></b><span>♧</span></header>
    ...
    <section id="pricing" class="compact-section public-pricing"><h2>{{ tr('pricing') }}</h2><div class="public-billing-controls"><select v-model="pricingCurrency" @change="pricingCurrencyEdited=true"><option value="EUR">EUR — €</option><option value="RUB">RUB — ₽</option><option value="UAH">UAH — ₴</option></select><div class="cycle"><button type="button" :class="{active:pricingCycle==='monthly'}" @click="pricingCycle='monthly'">{{tr('monthly')}}</button><button type="button" :class="{active:pricingCycle==='yearly'}" @click="pricingCycle='yearly'">{{tr('yearly')}}</button></div></div><div class="pricing-grid"><article v-for="plan in plans" :key="plan.id" class="price-card" :class="{ featured: plan.badge }"><div v-if="plan.image_url" class="plan-card-cover"><img :src="plan.image_url" :alt="plan.name"></div><span v-if="plan.badge" class="badge">{{ plan.badge }}</span><h3>{{ plan.name }}</h3><p>{{ plan.description }}</p><div class="price"><strong>{{ formatPlanPrice(plan) }}</strong><span>{{ pricingCycle==='yearly'?tr('perYear'):tr('perMonth') }}</span><em v-if="pricingCycle==='yearly'&&planSaving(plan)">{{tr('save')}} {{planSaving(plan)}}%</em></div><ul class="plan-feature-list"><li v-for="feature in plan.features" :key="feature.key" :class="{ disabled: !feature.included }"><span>{{ feature.included ? '✓' : '—' }}</span>{{ feature.label }}</li></ul><RouterLink class="button full" :to="`/${locale}/register?plan=${plan.id}`">{{ tr('choose') }}</RouterLink></article></div></section>
