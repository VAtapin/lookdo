...
const availableLocales = computed(
    () =>
        ...
        props.account?.tenant?.profile?.content?.enabled_locales || [
...
const localizedTagline = computed(
    () =>
        form.tagline_translations[props.locale] ||
        form.tagline_translations[mainLocale.value] ||
        form.tagline ||
...
const canConfirm = computed(() =>
    Boolean(
        localizedDescription.value.trim() &&
        logoUrl.value &&
        heroUrl.value &&
        form.service_modes.length,
    ),
);
const designConsultationIncluded = computed(
    () =>
        String(
            props.account?.entitlements?.design_consultation_included || "0",
        ) === "1",
);
const designRequestUrl = computed(() => {
    const body = [
        props.t("designRequestGreeting"),
        "",
        `${props.t("business")}: ${props.account?.tenant?.name || ""}`,
        `LOOKDO: ${props.account?.platform_url || location.origin}`,
        "",
        props.t("designRequestQuestions"),
    ].join("\n");
...
function brandingPayload() {
    ...
    return {
        ...
        business_description:
            form.description_translations[mainLocale.value] ||
            form.business_description,
        tagline: form.tagline_translations[mainLocale.value] || form.tagline,
    };
}
...
function normalizeViber(value: string) {
    const trimmed = String(value || "").trim();
    if (!trimmed || /^(?:https?:\/\/|viber:\/\/)/i.test(trimmed))
        return trimmed;
    const digits = internationalDigits(trimmed);
    return digits
...
async function upload(
    ...
    try {
        ...
        const result: any = await api(
            `/tenant/${props.tenantId}/branding/assets`,
            { method: "POST", body },
        );
        if (asset === "logo") logoUrl.value = result.url;
        else if (asset === "logo_horizontal")
            horizontalLogoUrl.value = result.url;
        else heroUrl.value = result.url;
        success.value = props.t("uploaded");
        emit("reload");
    } catch (e: any) {
        error.value = e.message;
    } finally {
...
async function generate() {
    ...
    try {
        const result: any = await api(
            `/tenant/${props.tenantId}/branding/generate`,
            {
                method: "POST",
                body: JSON.stringify({
                    asset: promptAsset.value,
                    prompt: prompt.value,
                }),
            },
        );
    ...
    <section class="mw-stack mw-branding">
        <header class="mw-page-head">
            <div>
                ...
                <h1>{{ t("branding") }}</h1>
                <p>
                    {{ t(onboarding ? "brandingOnboarding" : "brandingIntro") }}
                </p>
            </div>
        </header>
        <article class="mw-design-service">
            <div>
                <p class="mw-kicker">LOOKDO DESIGN</p>
                <h2>{{ t("designServiceTitle") }}</h2>
                <p>
                    {{
                        t(
                            designConsultationIncluded
            ...
            <a class="mw-primary" :href="designRequestUrl">{{
                t("contactLookdoDesigners")
            }}</a>
        </article>
        <p v-if="error" class="mw-error">{{ error }}</p>
        <p v-if="success" class="mw-success">{{ success }}</p>
        <div class="mw-branding-layout">
            <form class="mw-panel mw-branding-form" @submit.prevent="save()">
                <h2>{{ t("questionnaire") }}</h2>
                <fieldset class="mw-localized-fields">
                    <legend>{{ t("localizedTexts") }}</legend>
                    <div v-for="entry in availableLocales" :key="entry">
                        <b>{{ localeLabels[entry] || entry.toUpperCase() }}</b
                        ><label
                            >{{ t("businessDescription")
                            }}<textarea
                                v-model="form.description_translations[entry]"
                                rows="3"
                            ></textarea></label
                        ><label
                            >{{ t("tagline")
                ...
                <fieldset>
                    <legend>{{ t("serviceModes") }}</legend>
                    <label class="mw-check"
                        ><input
                            v-model="form.service_modes"
                            type="checkbox"
                            value="workshop"
                        />{{ t("serviceModeWorkshop") }}</label
                    ><label class="mw-check"
                        ><input
                            v-model="form.service_modes"
                            type="checkbox"
                            value="on_site"
                        />{{ t("serviceModeOnSite") }}</label
                    >
                </fieldset>
                <label
                    >{{ t("services")
                    }}<textarea v-model="form.services" rows="3"></textarea>
                </label>
                <label
                    >{{ t("targetCustomers")
                    }}<textarea v-model="form.customers" rows="3"></textarea>
                </label>
                <label
                    >{{ t("visualStyle") }}<input v-model="form.style"
                /></label>
                ...
                /></label>
                <fieldset class="mw-contact-fields">
                    <legend>{{ t("contacts") }}</legend>
                    <p>{{ t("contactsHint") }}</p>
                    <div class="mw-field-pair">
                        <label
                            >WhatsApp<input
                                v-model="form.whatsapp_url"
                                type="text"
                                inputmode="tel"
                                placeholder="+49 174 4812109"
                            /><small>{{ t("whatsappHelp") }}</small></label
                        ><label
                            >MAX<input
                                v-model="form.max_url"
                                type="url"
                                placeholder="https://max.ru/u/..."
                            /><small>{{ t("maxHelp") }}</small></label
                        >
                    </div>
                    <div class="mw-field-pair">
                        <label
                            ...
                            /><small>{{ t("telegramHelp") }}</small></label
                        ><label
                            >Viber<input
                                v-model="form.viber_url"
                                type="text"
                                inputmode="tel"
                                placeholder="+49 174 4812109"
                            /><small>{{ t("viberHelp") }}</small></label
                        >
                    </div>
                    <div class="mw-field-pair">
                        <label
                            >VK<input
                                v-model="form.vk_url"
                                type="url"
                                placeholder="https://vk.com/..." /></label
                        ><label
                            >Instagram<input
                ...
                <label
                    >{{ t("workingHours") }}<input v-model="form.working_hours"
                /></label>
            ...
            <div class="mw-stack">
                <article class="mw-panel mw-brand-assets">
                    <h2>{{ t("logo") }}</h2>
                    <div
                        class="mw-logo-preview"
                        :class="{
                            'mw-asset-preparing': preparingAsset === 'logo',
                        }"
                    >
                        <img v-if="logoUrl" :src="logoUrl" alt="" /><LineIcon
                            v-else
                            name="photo"
                        /><span v-if="preparingAsset === 'logo'"
                    ...
                    <div class="mw-asset-actions">
                        <label class="mw-secondary"
                            >{{ t("upload")
                                ...
                                @change="upload('logo', $event)" /></label
                        ><button
                            class="mw-primary"
                            :disabled="busy"
                            @click="prepare('logo')"
                ...
                <article class="mw-panel mw-brand-assets">
                    <h2>{{ t("heroImage") }}</h2>
                    <div
                        class="mw-hero-preview"
                        :class="{
                            'mw-asset-preparing': preparingAsset === 'hero',
                        }"
                    >
                        <img v-if="heroUrl" :src="heroUrl" alt="" /><LineIcon
                            v-else
                            name="photo"
                        /><span v-if="preparingAsset === 'hero'"
                            ><i></i>{{ t("preparingPrompt") }}</span
                        >
                    </div>
                    <div class="mw-asset-actions">
                        <label class="mw-secondary"
                            >{{ t("upload")
                                ...
                                type="file"
                                accept="image/jpeg,image/png,image/webp"
                                @change="upload('hero', $event)" /></label
                        ><button
                            class="mw-primary"
                            :disabled="busy"
                            @click="prepare('hero')"
        ...
        <article
            class="mw-brand-preview"
            :style="
                heroUrl
                    ? {
                          backgroundImage: `linear-gradient(180deg,transparent,#050607 88%),url('${heroUrl}')`,
                      }
                    : {}
            "
        >
            <header>
                <img
                    v-if="horizontalLogoUrl || logoUrl"
                    :src="horizontalLogoUrl || logoUrl"
                    alt=""
        ...
        <button
            v-if="onboarding"
            class="mw-primary mw-confirm-branding"
        ...
        <div v-if="promptOpen" class="mw-brand-modal" @click.self="closePrompt">
            <form class="mw-panel" @submit.prevent="generate">
                <header>
                    <div>
                        <p class="mw-kicker">{{ t("generateAi") }}</p>
                        <h2>{{ t("checkPrompt") }}</h2>
                    </div>
                    <button type="button" :disabled="busy" @click="closePrompt">
                        ×
                    </button>
                </header>
                <textarea
