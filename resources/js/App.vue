...
function switchLocale(value:Locale){setLocale(value);if(publicPage.value){const parts=route.path.split('/').filter(Boolean);if(['de','en','ru'].includes(parts[0]))parts.shift();router.push('/'+value+(parts.length?'/'+parts.join('/'):'/'));}}
</script>
<template>
  <div class="site-shell">
    <header v-if="showChrome" class="topbar">
      <RouterLink class="brand" :to="`/${locale}`"><span class="brand-mark">L·D</span><span>LOOKDO<small>LOOK. DO.</small></span></RouterLink>
