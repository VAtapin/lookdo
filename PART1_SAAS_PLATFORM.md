...
Если у выбранного тарифа `trial_days > 0`, регистрация сразу активирует тестовый период без обязательной предварительной оплаты. До даты окончания trial tenant получает единый максимальный набор возможностей всех активных публичных тарифов, чтобы проверить полную версию и затем выбрать Pro или Business. В кабинете явно показываются статус теста, оставшиеся дни и предложение выбрать тариф; тестовый доступ не считается оплаченной подпиской. Открытие Stripe Checkout во время trial не отключает приложение. После окончания trial без активной, комплиментарной или оплаченной подписки публичное приложение блокируется.

В супер-админке технический статус аккаунта отделён от статуса доступа: активный tenant не считается оплатившим. Доступ отображается как тестовый, оплаченный, вручную предоставленный, просроченный или неоплаченный. Супер-администратор может выдать клиенту ручной доступ без оплаты на заданное число дней; он хранится как manual/complimentary, автоматически заканчивается в указанную дату и никогда не учитывается как платёж или MRR. Смена тарифа администратором также не переводит неоплаченную подписку в `active`.
