...
Команда `lookdo:reminders:send` обрабатывает запланированные напоминания и фиксирует фактический результат канала. Она намеренно не подключается к scheduler автоматически: включение регулярной внешней отправки требует отдельного явного решения владельца платформы.
