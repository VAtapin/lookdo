...
- Ivanna Brows — тщательно заполненная конфигурация поверх общего beauty/brows template; для этого tenant включены украинский, русский и немецкий, английский остаётся доступен общему шаблону, но не показывается Иванне.
...
Технический порядок подключения: `config/tenant_apps.php` задаёт общий template, `config/tenant_presets.php` — начальный персональный overlay, а `profile.content.app_configuration` и остальные tenant-таблицы хранят дальнейшие изменения владельца. Preset применяется командой `lookdo:tenant-preset PRESET --tenant=SLUG`; повторный запуск не создаёт второй tenant или второй шаблон. `--force` обновляет только управляемые preset-поля и стартовые услуги, не подменяя идентификатор tenant.
