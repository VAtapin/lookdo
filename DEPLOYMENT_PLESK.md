...
Scheduler каждую минуту обрабатывает наступившие напоминания, ежедневно создаёт backup в 02:30 и проверяет последнюю копию в 04:00. Push отправляется непосредственно обработчиком напоминаний. SMS сначала записывается в очередь, поэтому для неё дополнительно должен постоянно работать указанный выше queue worker.

Фактическое состояние видно в Super Admin → `SMS-Protokoll`: время последнего запуска планировщика, готовность VAPID/seven.io, число ожидающих напоминаний и заданий очереди. Если после установки блок `Planer` остаётся красным дольше трёх минут, Scheduled Task Plesk не запущен или использует неверный PHP-путь.
